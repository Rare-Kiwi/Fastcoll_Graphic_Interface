...
inline uint32 xrng64()        //create 64 bits of ramdon unsigned int
{
...
inline uint32 II(uint32 b, uint32 c, uint32 d)
{	return c ^ (b | ~d); }
inline uint32 RL(uint32 x, unsigned int n)      //x * 2^n
{	return (x << n) | (x >> (32-n)); }
inline uint32 RR(uint32 x, unsigned int n)      //x \ 2^n
{	return (x >> n) | (x << (32-n)); }
inline ios_base&
  hex(ios_base& __base)
  {
...
#define Qoff 3 //num of set IHV




//指定输出文件
string outfile1="result_01";
string outfile2="result_02";
unsigned load_block(istream& i, uint32 block[]){
    unsigned len = 0;
        char uc;
...
void save_block(ostream& o, const uint32 block[])//结果文件名，文件内容
{
    for (unsigned k = 0; k < 16; ++k)
            for (unsigned c = 0; c < 4; ++c)
...
void find_collision(const uint32 IV[], uint32 msg1block0[], uint32 msg1block1[], uint32 msg2block0[], uint32 msg2block1[]){
        ...
        md5_compress(IHV, msg1block0);		// acquire ihv'
        //IHV2[25]=1 && IHV3[25]=0
...
void md5_compress(uint32 ihv[], const uint32 block[]){
     ...
     *
     */
        MD5_STEP(FF, a, b, c, d, block[0], 0xd76aa478, 7); //a = b + ((a + F(b,c,d) + block[0] + 0xd76aa478) <<< 7)
        ...
        ihv[2] += c;
        ihv[3] += d;
        //finally,MD5=A+B+C+D by big-endian (128bit)
}
void find_block0(uint32 block[], const uint32 IV[]){
    uint32 Q[68] = { IV[0], IV[3], IV[2], IV[1] };   //a working state

        std::vector<uint32> q4mask(1 << 4); 			//create 16 to storage
        for (unsigned k = 0; k < q4mask.size(); ++k)
            q4mask[k] = ((k<<2) ^ (k<<26)) & 0x38000004;  //initialized

        std::vector<uint32> q9q10mask(1<<3);			  //create 8 to storage
        for (unsigned k = 0; k < q9q10mask.size(); ++k)
            q9q10mask[k] = ((k<<13) ^ (k<<4)) & 0x2060;

        std::vector<uint32> q9mask(1<<16);				 //create 2^16 to storage
        for (unsigned k = 0; k < q9mask.size(); ++k)
            q9mask[k] = ((k<<1) ^ (k<<2) ^ (k<<5) ^ (k<<7) ^ (k<<8) ^ (k<<10) ^ (k<<11) ^ (k<<13)) & 0x0eb94f16;

        while (true)//search block 1
        {
            //addition of Q to set a sufficient conditions so as to find collision
            //Choose Q1,Q2,...,Q16 fulfilling conditions
            Q[Qoff + 1] = xrng64();
            ...
            while (counter < (1 << 7)) //counter < 128
            {
                ...
                if (0x00020000 != ((q18 ^ q17) & 0xa0020000)) // the condition of q_18 : the 31th and 29th must same as q_17 and 17th must not
                    continue;

                uint32 q19 = GG(q18, q17, q16) + tt19;
                q19 = RL(q19, 14); q19 += q18;
                if (0x80000000 != (q19 & 0x80020000)) // the condition of q_19 : 31th must 1 and 17th must 0
                    continue;

                uint32 q20 = GG(q19, q18, q17) + tt20;
                q20 = RL(q20, 20); q20 += q19;
                if (0x00040000 != ((q20^q19) & 0x80040000))	  // the condition of q_20 : 31th must same as q_19 and 18th must mot
                    continue;
            ...
            if (counter != 0) //still fail to get fit figures after 128 loops
                continue;
            ...
            unsigned counter2 = 0;
            while (counter2 < (1<<4)) //the times of ceil is 16
            {
                ...
                for (unsigned counter3 = 0; counter3 < (1<<3);)
                {
                    ...
                    for (unsigned counter4 = 0; counter4 < (1<<16); ++counter4)
                    {
                        ...
                        uint32 a = aa, b = bb, c = cc, d = dd; //another sort of ihv
                        MD5_STEP(GG, a, b, c, d, block[9], 0x21e1cde6, 5);
                        MD5_STEP(GG, d, a, b, c, block[14], 0xc33707d6, 9);
                        MD5_STEP(GG, c, d, a, b, block[3], 0xf4d50d87, 14);
                        MD5_STEP(GG, b, c, d, a, block[8], 0x455a14ed, 20);
